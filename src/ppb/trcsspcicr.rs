...
#[doc = "Field `PC` reader - Selects one or more PE comparator inputs for Single-shot control. TRCIDR4.NUMPC defines the size of the PC field. 1 bit is provided for each implemented PE comparator input. For example, if bit\\[1\\] == 1 this selects PE comparator input 1 for Single-shot control"]
pub type PC_R = crate::FieldReader;
#[doc = "Field `PC` writer - Selects one or more PE comparator inputs for Single-shot control. TRCIDR4.NUMPC defines the size of the PC field. 1 bit is provided for each implemented PE comparator input. For example, if bit\\[1\\] == 1 this selects PE comparator input 1 for Single-shot control"]
pub type PC_W<'a, REG> = crate::FieldWriter<'a, REG, 4>;
impl R {
    #[doc = "Bits 0:3 - Selects one or more PE comparator inputs for Single-shot control. TRCIDR4.NUMPC defines the size of the PC field. 1 bit is provided for each implemented PE comparator input. For example, if bit\\[1\\] == 1 this selects PE comparator input 1 for Single-shot control"]
    #[inline(always)]
    pub fn pc(&self) -> PC_R {
